...
| L4 - Interview performance | Concise frameworks, "how I'd explain this to a PM," comparison questions, story prompts. | Interview Q&A + Quick Reference + Mind Map |
...
| Mind Map | Night-before recall: mermaid overview plus a dense outline of every concept already in the folder. Filename `{Topic} - Mind Map.md`. |
...
6. How to build it safely - Controls in order of impact (design > config > code).
...
5. If the topic is a heavy hitter (OAuth, TLS, IAM, threat modeling), add a Mastery Track file and wire it in `topics.json`.
6. After the guides exist, add `{Topic} - Mind Map.md`. I pull headings from the Comprehensive Guide, Quick Reference, Critical Clarification, and VAPT, then run `Scripts/audit_mindmap_coverage.py` after the draft and again after I reread the clarifications. Skip navigational entries (`quick-start-guide`, `study-plan`) and the legacy HttpOnly Q&A alias.
